...
function getShifts() {
  ...
  fetch(url + `/shifts/${id}`)
    ...
    .then((shifts) =>
      printList(
        id ? "list of current worker shifts:" : "all workers shifts:",
        shifts
      )
    );
}
...
function getHours() {
  ...
  fetch(url + `/hours/${id}`)
    ...
    .then((hours) => {
      printValue(
        id
          ? "amount of current worker hours worked:"
          : "sum of all workers hours worked:",
        hours
      );
    });
}
...
function getSalary() {
  ...
  fetch(url + `/salary/${id}`)
    ...
    .then((hours) => {
      printValue(
        id ? "current worker salary:" : "sum of all workers salaries:",
        hours
      );
    });
}
...
function punchIn() {
  ...
  console.log(`fetching ${url}/punchtime/${id}/${PUNCH_IN}/${dateTimeNum}`);
  fetch(`${url}/punchtime/${id}/${PUNCH_IN}/${dateTimeNum}`)
    .then((res) => res.json())
    .then((worker) => {
      printWorker(worker);
      if (worker.id) {
...
function punchOut() {
  const id = document.getElementById("idInp").value;
  const dateTimeNum = getChosenTime();
  fetch(`${url}/punchtime/${id}/${PUNCH_OUT}/${dateTimeNum}`)
    .then((res) => res.json())
...
const MainForm = () => {
  return (
    <React.Fragment>
      ...
      <button id="inBtn" onClick={punchIn}>
        punch in
      </button>
      <button id="outBtn" onClick={punchOut}>
        punch out
      </button>
